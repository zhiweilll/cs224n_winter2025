...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int], grader_params=None) -> Tuple[
        ...
        # step 2: post_embed_cnn
        X = torch.permute(X, (1,2,0))
        X = self.post_embed_cnn(X)
        X = torch.permute(X, (2,0,1))
        # step 3: apply encoder
        ...
        init_decoder_hidden = self.h_projection(last_hidden_combined)
        last_cell_combined = torch.cat((last_cell[0], last_cell[1]), dim = 1)
        init_decoder_cell = self.c_projection(last_cell_combined)
