...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # 1) embed_to_hidden layer
        self.embed_to_hidden_weight = nn.Parameter(torch.empty(self.n_features * self.embed_size, self.hidden_size)) # W
        nn.init.xavier_uniform_(self.embed_to_hidden_weight)
        self.embed_to_hidden_bias = nn.Parameter(torch.empty(self.hidden_size)) # b1
        nn.init.uniform_(self.embed_to_hidden_bias)

        # 2) dropout layer
        self.dropout = nn.Dropout(self.dropout_prob)

        # 3) hidden_to_logits layer
    ...
    def embedding_lookup(self, w):
        ...
        x = self.embeddings[w]
        x = x.flatten(start_dim=1) # (batch_size, n_features * embed_size)
