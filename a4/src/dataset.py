...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_len = random.randint(1, trunc_len//2) # mask_len ~ Uniform(1, trunc_len/2)， E is trunc_len/4
        mask_start = random.randint(0, trunc_len - mask_len) # allow prefix and suffix to be empty
        prefix = trunc_document[:mask_start]
        masked_content = trunc_document[mask_start:mask_start + mask_len]
        suffix = trunc_document[mask_start + mask_len:]

        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        masked_string = masked_string + self.PAD_CHAR*(self.block_size + 1 - len(masked_string)) # total en is block_size + 1
