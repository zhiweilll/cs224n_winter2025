...
if args.variant == 'vanilla':
    ...
    ### YOUR CODE HERE ###
    mconf.rope = True # rope is a element from models.GPTConfig class: if True, no pos_emb will be applied
    model = models.GPT(mconf).to(device)
    ### END YOUR CODE ###
else:
...
if args.function == 'pretrain':
    ...
    tconf = trainer.TrainerConfig(
        ...
        warmup_tokens=512 * 20,
        final_tokens=650 * len(pretrain_dataset) * block_size,
        num_workers=4, # 4 for multiprocessing
        writer=writer
    )
    ...
    print(f'pretrained model saved to {args.writing_params_path}')
    ### END YOUR CODE ###
    ...
    if args.reading_params_path is None:
        # finetune without a pretrained model
        tconf = trainer.TrainerConfig(
            max_epochs=75,
            batch_size=256,
            learning_rate=args.finetune_lr,
        ...
        trainer.train()
        print(f'Finetune without a pretrained model completed')
    else:
        ...
        trainer = trainer.Trainer(model, finetune_dataset, None, tconf)
        trainer.train()
        print(f'Finetune with a pretrained model completed')
    torch.save(model.state_dict(), args.writing_params_path)
    print(f'finetuned model saved to {args.writing_params_path}')
    ### END YOUR CODE ###
